      ...
      <section id="projects" class="portfolio-section reveal-section" data-section-index="2" aria-labelledby="projects-heading">
        <div class="container">
            <div class="projects-wrapper">
              <div class="section-header"><span class="section-subtitle">{{ copy.myWork }}</span><h2 id="projects-heading" class="section-title">{{ copy.projectsTitle }}</h2></div>
              <div class="project-category-tabs" role="tablist" :aria-label="projectCategoryCopy.label">
                <button id="professional-projects-tab" class="project-category-tab" :class="{ active: activeProjectCategory === 'professional' }" type="button" role="tab" :aria-selected="activeProjectCategory === 'professional'" aria-controls="projects-carousel-panel" @click="activeProjectCategory = 'professional'">{{ projectCategoryCopy.professional }}</button>
                <button id="independent-projects-tab" class="project-category-tab" :class="{ active: activeProjectCategory === 'independent' }" type="button" role="tab" :aria-selected="activeProjectCategory === 'independent'" aria-controls="projects-carousel-panel" @click="activeProjectCategory = 'independent'">{{ projectCategoryCopy.independent }}</button>
              </div>
              <div id="projects-carousel-panel" role="tabpanel" :aria-labelledby="activeProjectCategory === 'professional' ? 'professional-projects-tab' : 'independent-projects-tab'">
                <ProjectCarousel3D :key="`${language}-${activeProjectCategory}`" :projects="visibleProjects" :language="language" />
              </div>
            </div>
        </div>
      </section>
...
const localizedPortfolio = computed(() => language.value === 'pt' ? portfolioData : { ...portfolioData, profile:{ ...portfolioData.profile, ...portfolioContentEn.profile }, experiences:portfolioContentEn.experiences, education:portfolioContentEn.education, courses:portfolioContentEn.courses, projects:portfolioContentEn.projects, independentProjects:portfolioContentEn.independentProjects });
const visibleProjects = computed(() => activeProjectCategory.value === 'professional' ? localizedPortfolio.value.projects : localizedPortfolio.value.independentProjects);
